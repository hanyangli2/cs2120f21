...
The following propisition is true.
-/
def bij_trans (s : β → γ → Prop)  (r : α → β → Prop) :
  bijective r → bijective s → bijective (composition s r) := 
  begin 
    unfold bijective surjective injective composition total_function function single_valued defined,
    assume h,
    assume j,
    cases h,
    cases h_left,
